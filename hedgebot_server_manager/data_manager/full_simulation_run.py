...
def main(initial_simulation_variables, prev_year_financial_df, mc_meta_data_1):
    ...
    for i in range(0,10000):
        ...
        final_df['Mean Returned'] = pd.to_numeric(final_df.mean_returned.astype(str).str.replace(',',''), errors='coerce').fillna(0).astype(int)

        temp = models.current_financial_simulations.objects.create(
            simulation_number = i,
